...
> ⚠️ This is a planning document. No implementation code has been written.
> **Complexity: Advanced** — financial compliance, external payment gateway, VIP tier logic, multi-branch decision tree, audit trail, circuit breaker.
...
- Mediator: MediatR — all use cases are Command/Query handlers
- Test framework: xUnit + NSubstitute + FluentAssertions
- Validation: FluentValidation — validators auto-registered via `AddValidatorsFromAssembly()`
- DI registration: Via `AddOrderModule()` extension in `src/Features/Orders/OrderModule.cs`
- Error handling: `Result<T>` pattern with `Error` value object — never throws for business rule violations, only for infrastructure failures
- API pattern: Minimal APIs with `TypedResults` — each feature has an `Endpoint.cs` that maps routes
...
## Prohibitions (Don'ts)
- ❌ NEVER auto-approve a cancellation where the refund exceeds $500 — queue for manager review. This is a compliance requirement.
- ❌ NEVER call the payment gateway directly — all refunds go through `IPaymentMediator`. Direct gateway calls bypass idempotency and audit logging.
- ❌ NEVER return raw exception details to the API caller — use the `Result<T>` pattern for business errors and `ProblemDetails` for infrastructure errors.
- ❌ NEVER process a refund without an `IdempotencyKey` — duplicate refunds have caused real financial loss in the past.
- ❌ NEVER send emails or HTTP calls inside a DB transaction — publish a `CancellationCompletedEvent` and let the event handler send notifications.
- ❌ NEVER trust the client-supplied order amount for refund calculation — always re-fetch from `IOrderRepository`.
- ❌ NEVER log full credit card numbers, customer email addresses, or auth tokens in cancellation logs — log only masked card suffix and customer ID.
- ❌ NEVER use the legacy `OrderService` class — it's deprecated and scheduled for removal. Use the MediatR handler pattern instead.
...
│   ├── Customer doesn't own order → 403 Forbidden
│   ├── Order already cancelled → 409 Conflict
│   └── Validation passes → Continue
│
├── Customer is VIP?
│   └── YES → Flag for priority handling (never auto-decline VIP)
│
├── Order Status?
│   ├── Processing (not shipped)
│   │   └── Full refund → Cancel immediately
│   ├── Shipped (not delivered)
│   │   └── Initiate return → Partial refund (minus shipping cost)
│   ├── Delivered ≤ 30 days (90 for VIP)
│   │   └── Require return request first → Queue, don't process yet
│   └── Delivered > 30 days (90 for VIP)
│       └── Decline refund → Offer store credit as goodwill
│
├── Calculated refund > $500?
│   └── YES → Queue for manager review (never auto-approve)
│
└── Payment gateway available?
    ├── YES → Process refund via IPaymentMediator
    └── NO → Queue with retry policy → Alert ops after 3 failures
```
...
- 🛑 **Fail if**: Order not found, customer doesn't own order, idempotency conflict detected, refund amount doesn't match recalculated total (possible tampering)
- ⏸️ **Queue for review if**: Refund amount > $500, customer flagged for fraud review, VIP customer (priority queue), return window is borderline (within 48h of expiry)
- 🔄 **Retry with backoff if**: Payment gateway returns 503 — max 3 attempts with exponential backoff (2s, 4s, 8s), then queue for manual processing
- 🔔 **Alert if**: Gateway failure rate exceeds 10% in 5-minute window (circuit breaker via Polly), any refund amount calculation mismatch detected

## Data Model
- New entity: `CancellationRequest` — belongs to `Order`, references `Customer`
  - Properties: `Id`, `OrderId`, `CustomerId`, `Reason`, `RequestedAt`, `Status` (enum: Pending, Approved, Declined, PendingReview), `RefundAmount`, `RefundMethod`, `IdempotencyKey`, `DecisionPath` (string — records which branch was taken), `ReviewedBy` [NEEDS CLARIFICATION: Is this a UserId, or a role/team name? Depends on how manager review queue is implemented], `ReviewedAt`
- New enum: `CancellationStatus` in `src/Features/Orders/Domain/`
- Modified: `Order` entity — add `CancellationRequestId` (nullable FK)
- Migration needed: Yes — new `CancellationRequests` table + nullable FK column on `Orders`

## API Contract
- Route: `POST /api/orders/{orderId}/cancel`
- Auth: Bearer token, must own the order (check via `ICurrentUserService.UserId`)
- Request body fields:
  - `reason` (string, required, max 500 chars) — customer's stated reason for cancellation
  - `refundPreference` (string, required, one of: `original-payment-method`, `store-credit`) — where the refund should go
- Success responses:
  - `200 OK` (immediate approval) — returns cancellationId (guid), status ("approved"), refundAmount (decimal), refundMethod (string)
  - `202 Accepted` (queued for review) — returns cancellationId (guid), status ("pending-review"), estimatedReviewTime (string, e.g. "24h")
- Error responses:
  - `403 Forbidden` — customer doesn't own this order
  - `404 NotFound` — order doesn't exist
  - `409 Conflict` — order already cancelled
  - `422 UnprocessableEntity` — validation failure (missing reason, invalid preference)
...
- [ ] Given a refund amount of $750, when auto-approve is attempted, then it MUST return status `pending-review` — never `approved`
- [ ] Given a cancellation request, then the payment gateway is NEVER called directly — only `IPaymentMediator.SubmitRefundAsync()` is invoked
- [ ] Given any cancellation, then the logs NEVER contain customer email, full card number, or auth tokens
- [ ] Given a cancellation, then no email is sent synchronously — only a domain event is published
...
- Create: `src/Features/Orders/CancelOrder/CancelOrderCommand.cs` — command record with OrderId, Reason, RefundPreference, IdempotencyKey
- Create: `src/Features/Orders/CancelOrder/CancelOrderHandler.cs` — main handler with decision tree logic; follow the pattern in `PlaceOrderHandler.cs`
- Create: `src/Features/Orders/CancelOrder/CancelOrderValidator.cs` — FluentValidation rules; follow the pattern in `PlaceOrderValidator.cs`
- Create: `src/Features/Orders/CancelOrder/CancelOrderEndpoint.cs` — minimal API endpoint mapping; follow the pattern in `PlaceOrderEndpoint.cs`
- Create: `src/Features/Orders/Domain/CancellationRequest.cs` — new entity with properties listed in Data Model
- Create: `src/Features/Orders/Domain/CancellationStatus.cs` — enum: Pending, Approved, Declined, PendingReview
- Create: `src/Features/Orders/Events/CancellationCompletedEvent.cs` — domain event for async notification and audit
- Create: `tests/Features/Orders/CancelOrder/CancelOrderHandlerTests.cs` — unit tests covering all acceptance criteria; follow test naming in `PlaceOrderHandlerTests.cs`
- Create: `tests/Features/Orders/CancelOrder/CancelOrderValidatorTests.cs` — validation edge cases
- Modify: `src/Features/Orders/Domain/Order.cs` — add nullable `CancellationRequestId` FK property
- Modify: `src/Features/Orders/OrderModule.cs` — register handler, validator, and event handler in DI
- Modify: `src/Infrastructure/Persistence/AppDbContext.cs` — add `DbSet<CancellationRequest>`, configure relationship in `OnModelCreating`
...
- Log (Error): Refund amount mismatch detected (expected vs calculated — possible tampering)
